...
def detect_SWR(eeg, eeg_fs, ied_bool, data_dropout,rippleBand=[80, 150], gaussFilt=10.0, zThresh=3, durTresh=(0.03, 0.15) ):
    ...
    #get the invalid indices IED times and data dropout (need to load that)
    ...
    #need to get eeg at valid regions -> create a mask and then when using the smoothed_ripple_amplitude only use the mask
    

    #filter EEG in ripple band
    ...
    #gaussian smoothing -> can use scipy function
    ...
    for region in selected_regions:
        start, end = region.bbox[1], region.bbox[3]  # Get region start and end indices
        output["rippleTime"].append(region.coords[:, 1] / eeg_fs)
